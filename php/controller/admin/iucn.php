<?php
if (!count($_POST)) { include DIR."/php/view/admin/iucn.php"; die(); }

use Trust\JSONResponse;
use SSBIN\IUCN_status;
use Trust\Forms;
use Trust\Pager;
...
function get() {
  ...
  $p->statuses = IUCN_status::allWhere("$p->strWhere $p->strOrder $p->strLimit", []);
  $p->totalItems = IUCN_status::countWhere($p->strWhere, []);
...
function saveOld() {
  ...
  $check = IUCN_status::where('WHERE abbr=:new AND abbr<>:old', ['new'=>$o->abbr,'old'=>$target->abbr]);
  if ($check != null) JSONResponse::Error("The code $o->abbr already exists");

  $update = IUCN_status::find($target->abbr);
  $update->abbr = $o->abbr;
  $update->long_name = $o->long_name;
  $update->setTimestamps();
  //Note: Belum bisa $update->save() karena mau update PK juga.
  \Trust\DB::exec('UPDATE iucn_status SET abbr=:abbr, long_name=:long_name, data_info=:info WHERE abbr=:target',
    [
      'abbr'=>$o->abbr,
