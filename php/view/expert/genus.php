<?php
$pageTitle = "Taxonomy Genus";
$pageSubTitle = "Manage Genus";
include $template;
...
function htmlHead() { ?>
  ...
  <script src="/js/expert/genus.js"></script>
  <script src="/jslib/mine/DirPaging2.js"></script>
<?php }

function mainContent() { ?>
  <div class="row" ng-controller="ctrlGenus">
    <div class="col-md-6">
      ...
      <table class="table table-bordered table-striped table-condensed table-responsive">
        ...
        <tbody>
          <tr ng-repeat="o in genus">
            <td>{{$index + pager.startRecord()}}</td>
            <td><a ng-click="edit(o)">{{o.genus}}</a></td>
          </tr>
        </tbody>
      </table>
            ...
            <div class="modal-body">
              <table>
                <tr>
                  <td>Genus</td>
                  <td>
                    <input type="text" ng-model="o.genus" class="form-control" list="catList"/>
                    <datalist id="catList"><option ng-repeat="c in genusList" ng-change="oChanged()" value="{{c}}"></option></datalist>
                  </td>
                </tr>
              </table>
  ...
  <div id="init" style="display:none"><?php
    $init = new stdClass();
    $init->genus = \SSBIN\Genus::allWhere(" ORDER BY genus LIMIT 50", []);
    $init->totalItems = \SSBIN\Genus::count();
