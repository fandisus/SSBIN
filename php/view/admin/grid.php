...
function mainContent() { ?>
  <div class="row" ng-controller="ctrlGrid">
    <div class="col-md-6">
      ...
      <table class="table table-bordered table-striped table-condensed table-responsive">
        ...
        <tbody>
          <tr ng-repeat="o in grids">
            <td>{{$index + pager.startRecord()}}</td>
            <td><a ng-click="edit(o)">{{o.grid}}</a></td>
          </tr>
        </tbody>
      </table>
            ...
            <div class="modal-body">
              <table>
                <tr>
                  <td>Grid</td>
                  <td>
                    <input type="text" ng-model="o.grid" class="form-control" list="catList"/>
                    <datalist id="catList"><option ng-repeat="c in gridList" ng-change="oChanged()" value="{{c}}"></option></datalist>
                  </td>
                </tr>
              </table>
  ...
  <div id="init" style="display:none"><?php
    $init = new stdClass();
    $init->grids = \SSBIN\Grid::allWhere(" ORDER BY grid LIMIT 50", []);
    $init->totalItems = \SSBIN\Grid::count();
